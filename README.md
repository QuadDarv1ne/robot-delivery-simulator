...
Комплексный 3D симулятор робота-доставщика с интеграцией Unity WebGL,
поддержкой ROS/ROS2 и визуализацией сенсоров в реальном времени. Подходит для
образовательных целей, исследований в области робототехники и разработки
алгоритмов.

![Превью симулятора](./img/2026-03-13-v1.png)
...
- **Данные сенсоров в реальном времени** — GPS, Lidar, IMU,
  энкодеры с частотой 10 Гц
...
- **3D облако точек Lidar** — Визуализация на Three.js с цветовой кодировкой
  по расстоянию
...
```bash
# Проверьте DATABASE_URL и примените актуальную схему
npx prisma db push
npm run seed
```
...
Приложение использует Prisma с PostgreSQL. Основные модели:
...
`docker/Dockerfile` и откроет порт `3000`.
...
См. [конфигурацию Railway](./railway.json) и [Docker-конфигурацию](./docker/Dockerfile).
...
[Руководством по участию](./CONTRIBUTING.ru.md).
